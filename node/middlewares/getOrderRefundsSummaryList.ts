...
export async function getOrderRefundsSummaryList(ctx: Context) {
  ...
  const orderListPromises: ordersToSummary[] = []
   ...
   for (const order of body) {
    ...
    const hasQuantityGreaterThanZero = response.items.map(item => item.amountAvailablePerItem.quantity).some(quantity => quantity > 0);
    const hasRefunds = response.transactions ? response.transactions.length > 0 ? true : false : false
    const orderToSummary = {
      ...order,
      hasRefunds: hasRefunds,
      hasAmount: response.amountsAvailable.shipping > 0 || response.amountsAvailable.order > 0 || hasQuantityGreaterThanZero ,
    }
